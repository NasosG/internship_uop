...
const insertStudentPositionsFromUser = async (studentId, positionId, priority, atlas) => {
  try {
    ...
    const positionInfo = await pool.query("SELECT name as company, afm, title, city, last_update_string FROM"
      ...
      + ` WHERE (pos.atlas_position_id IS NULL AND pos.id = ${positionId}) or  pos.atlas_position_id = ${positionId} `);
    ...
    console.log("1231231231asdasdas2");
...
const findIfPositionExists = async (studentId, positionId, atlas) => {
  try {
    const positionInfo = null;
    console.log("ASd");
    if (atlas)
      positionInfo = await pool.query("SELECT COUNT(*) as poscount " +
      ...
      positionInfo = await pool.query("SELECT COUNT(*) as poscount " +
        " FROM student_positions pos" +
        ` WHERE pos.internal_position_id = ${positionId}` +
        ` AND pos.student_id = ${studentId}`);
