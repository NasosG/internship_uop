...
const getDataOfeloumenou = async (studentInfo, position) => {
  const afm = studentInfo.afm;
  const amka = studentInfo.ssn;
  ...
  const genderProcessed = gender === 'female' ? 5302 : 5301;
  const dob = studentInfo.scacdateofbirth;
  const dobFormatted = formatDateString(dob);
  const stFlag = 1;
  const eisodosFlag = 1;
  ...
  const startTimeFormatted = formatDateString(startTime);
  const endTimeFormatted = formatDateString(endTime);
  const oloklirosi = 1;
