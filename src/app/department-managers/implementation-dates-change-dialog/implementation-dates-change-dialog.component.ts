...
@Component({
  selector: 'app-implementation-dates-change-dialog',
  templateUrl: './implementation-dates-change-dialog.component.html',
  styleUrls: ['./implementation-dates-change-dialog.component.css'],
  providers: [
...
export class ImplementationDatesChangeDialogComponent implements OnInit {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ImplementationDatesChangeDialogComponent>
  ) { }
  ...
  updateImplementationDates() {
    ...
    this.depManagerService.changeImplementationDatesAtlasByAssignedPositionId(this.data.assigned_position_id, implementationDatesArray)
      .pipe(
        catchError((error: any) => {
          console.error(error);
          Swal.fire({
            title: 'Σφάλμα',
            text: 'Προέκυψε κάποιο σφάλμα κατά την αλλαγή των ημερομηνιών εκτέλεσης ΠΑ στο ΑΤΛΑΣ.',
            icon: 'error',
