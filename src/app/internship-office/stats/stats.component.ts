    ...
    this.officeService.getAchievementsStatsForStudents()
      .subscribe((res: any) => {
        ...
        const dataForExcel = res.map((item: any, index: number) => {
          ...
          const publicCompanyKeywords = ['Γ.Ν.Ε', 'ΓΝΕ', 'Γ.Ν.Α', 'ΓΝΑ', 'ΔΗΜΟΣ', 'ΔΗΜΟΥ', 'ΠΕΡΙΦΕΡΕΙΑ', 'ΓΕΝΙΚΟ ΝΟΣΟΚΟΜΕΙΟ', 'ΠΓΝ', 'ΓΕΝΙΚΟ ΛΥΚΕΙΟ', 'ΑΝΕΞΑΡΤΗΤΗ ΑΡΧΗ', 'ΕΘΝΙΚΟ', 'ΔΙΕΥΘΥΝΣΗ', 'ΒΟΥΛΗ', 'ΔΗΜΟΤΙΚΟ', 'ΔΗΜΟΤΙΚΗ', 'ΥΠΟΥΡΓΕ', 'Υπουργείο', 'ΔΗΜ.', 'ΠΟΛΕΜΙΚΟ ΜΟΥΣΕΙΟ', 'Εφορεία Αρχαιοτήτων', 'ΕΦΟΡΕΙΑ', 'ΟΛΥΜΠΙΑΚΟ ΑΘΛΗΤΙΚΟ ΚΕΝΤΡΟ', 'ΙΝΣΤΙΤΟΥΤΟ ΠΛΗΡΟΦΟΡΙΚΗΣ', 'ΠΑΝΕΠΙΣΤΗΜΙΟ', 'ΠΑΝΕΠΙΣΤΗΜΙΑΚΟ ΝΟΣΟΚΟΜΕΙΟ', 'ΘΕΑΓΕΝΕΙΟ', 'ΒΕΝΙΖΕΛΕΙΟ', 'Ν.Π.Δ.Δ.', 'ΕΠΙΜΕΛΗΤΗΡΙΟ', 'Επιμελητήριο', 'ΠΡΑΣΙΝΟ ΤΑΜΕΙΟ', 'ΚΡΑΤΙΚΟ ΘΕΑΤΡΟ'];
